...
const index_wait_1 = require("./lib/index-wait");
...
async function runLevel1() {
    ...
    try {
        ...
        const upload = await (0, uploader_1.uploadFileWithTiming)(companyId, smallFilePath);
        if (!upload.success || !upload.jobId) {
            throw new Error('Upload failed');
        }
        const indexResult = await (0, index_wait_1.waitForIndexing)(upload.jobId, 60000);
        if (!indexResult.success) {
            throw new Error('Indexing failed');
        }
        await new Promise((r) => setTimeout(r, 2000));
        const searchRes = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, { query: 'refund policy', limit: 5 }, { headers: { 'x-api-key': API_KEY } });
...
async function runLevel2() {
    ...
    try {
        console.log('\n📝 Test 2.1: Cache Performance');
        const upload = await (0, uploader_1.uploadFileWithTiming)(companyId, TEST_FILE_2000);
        if (!upload.success || !upload.jobId) {
            throw new Error('Upload failed');
        }
        await (0, index_wait_1.waitForIndexing)(upload.jobId, 120000);
        await new Promise((r) => setTimeout(r, 2000));
        const query = 'What is the refund policy?';
        const start1 = Date.now();
        const search1 = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, { query, limit: 10 }, { headers: { 'x-api-key': API_KEY } });
        const time1 = Date.now() - start1;
        const cacheStatus1 = search1.headers['x-cache'] || 'UNKNOWN';
        const start2 = Date.now();
        const search2 = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, { query, limit: 10 }, { headers: { 'x-api-key': API_KEY } });
    ...
    try {
        ...
        const { recursiveChunkText } = await Promise.resolve().then(() => __importStar(require('../src/utils/text-processor')));
    ...
    try {
        console.log('\n📝 Test 2.3: Bull Board Dashboard');
        const response = await axios_1.default.get(`${API_URL}/admin/queues`, {
            validateStatus: () => true,
        });
...
async function runLevel3() {
    ...
    try {
        ...
        const upload1 = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/uploads`, form1, {
            headers: { ...form1.getHeaders(), 'x-api-key': API_KEY },
        });
        const fileId1 = upload1.data.fileId;
        await (0, index_wait_1.waitForIndexing)(upload1.data.jobId, 120000);
        const form2 = new form_data_1.default();
        form2.append('file', fs.createReadStream(TEST_FILE_3000));
        const upload2 = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/uploads`, form2, {
            headers: { ...form2.getHeaders(), 'x-api-key': API_KEY },
        });
        await (0, index_wait_1.waitForIndexing)(upload2.data.jobId, 120000);
        await new Promise((r) => setTimeout(r, 2000));
        const searchFiltered = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, {
            query: 'What is the refund policy?',
            limit: 10,
            filter: { fileId: fileId1 },
        }, { headers: { 'x-api-key': API_KEY } });
        const allFromFile1 = searchFiltered.data.results.every((r) => r.payload?.fileId === fileId1);
    ...
    try {
        ...
        for (let i = 1; i <= 105; i++) {
            const res = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, { query: 'test', limit: 1 }, {
                headers: { 'x-api-key': API_KEY },
                validateStatus: () => true,
            });
            if (res.status === 200) {
...
async function runLevel4() {
    ...
    try {
        ...
        const files = ['1mb.txt', '10mb.txt', '20mb.txt'].filter((f) => fs.existsSync(path.join(DATA_DIR, f)));
        if (files.length === 0) {
            throw new Error('No test data files found');
        }
        const filePaths = files.map((f) => path.join(DATA_DIR, f));
        const uploadResults = await (0, uploader_1.uploadBatch)(companyId, filePaths);
        const jobIds = uploadResults.filter((r) => r.success && r.jobId).map((r) => r.jobId);
        if (jobIds.length === 0) {
            throw new Error('No successful uploads');
        }
        // Large files need more time - increase timeout to 15 minutes
        const indexResults = await (0, index_wait_1.waitForBatch)(jobIds, 900000);
    ...
    try {
        console.log('\n📝 Test 4.2: Performance Metrics');
        await new Promise((r) => setTimeout(r, 2000));
        const metrics = await (0, metrics_1.gatherMetrics)(companyId, 'policy refund processing');
    ...
    try {
        console.log('\n📝 Test 4.3: Search with Large Collection');
        const searchRes = await axios_1.default.post(`${API_URL}/v1/companies/${companyId}/search`, { query: 'refund policy', limit: 10 }, { headers: { 'x-api-key': API_KEY } });
        if (searchRes.data.results && searchRes.data.results.length > 0) {
...
async function runLevel5() {
    ...
    try {
        ...
        const largeFiles = ['15mb.txt', '30mb.txt', '45mb.txt'].filter((f) => fs.existsSync(path.join(DATA_DIR, f)));
        if (largeFiles.length === 0) {
            throw new Error('No large test files found');
        }
        const filePath = path.join(DATA_DIR, largeFiles[0]);
        const upload = await (0, uploader_1.uploadFileWithTiming)(companyId, filePath);
        if (!upload.success || !upload.jobId) {
            throw new Error('Upload failed');
        }
        const indexResult = await (0, index_wait_1.waitForIndexing)(upload.jobId, 600000);
        if (!indexResult.success) {
            throw new Error('Indexing failed');
        }
        const fileSizeMB = fs.statSync(filePath).size / 1024 / 1024;
        const throughput = indexResult.chunks / (indexResult.durationMs / 1000);
        console.log(`   ✅ Test 5.1 PASSED (${fileSizeMB.toFixed(1)}MB, ${indexResult.chunks} chunks, ${throughput.toFixed(1)} chunks/sec)`);
        results.push({
    ...
    try {
        ...
        const filePaths = files.map((f) => path.join(DATA_DIR, f));
        const uploadPromises = filePaths.map((fp) => (0, uploader_1.uploadFileWithTiming)(companyId2, fp));
        const uploadResults = await Promise.all(uploadPromises);
        const jobIds = uploadResults.filter((r) => r.success && r.jobId).map((r) => r.jobId);
        if (jobIds.length < 2) {
            throw new Error('Not enough successful concurrent uploads');
        }
        const indexResults = await (0, index_wait_1.waitForBatch)(jobIds, 600000);
        const successCount = indexResults.filter((r) => r.success).length;
        if (successCount === jobIds.length) {
    ...
    try {
        ...
        const filePaths = files.map((f) => path.join(DATA_DIR, f));
        const uploadResults = await (0, uploader_1.uploadBatch)(companyId3, filePaths);
        const jobIds = uploadResults.filter((r) => r.success && r.jobId).map((r) => r.jobId);
        const indexResults = await (0, index_wait_1.waitForBatch)(jobIds, 600000);
        const totalChunks = indexResults.reduce((sum, r) => sum + r.chunks, 0);
        const successCount = indexResults.filter((r) => r.success).length;
...
async function runUnifiedTests() {
    ...
    const difficultyArg = args.find((a) => a.startsWith('--difficulty=')) || args.find((a) => a.startsWith('-d='));
    ...
    try {
        ...
        (0, metrics_1.writeReport)(result, reportFilename);
        // Exit code
