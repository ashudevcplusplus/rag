...
const OUT_DIR = path_1.default.join(__dirname, 'data');
...
function seededRandom(seed) {
    let h = crypto_1.default.createHash('sha256').update(seed).digest();
    let idx = 0;
    return () => {
        ...
        if (idx >= h.length) {
            ...
            h = crypto_1.default.createHash('sha256').update(h).digest();
            idx = 0;
        }
        return val / 0xffffffff;
    };
}
function generateFile(filename, targetBytes, seed = 'default') {
    const filePath = path_1.default.join(OUT_DIR, filename);
