...
const cache_service_1 = require("../../../src/services/cache.service");
...
describe('CacheService', () => {
    ...
    describe('generateKey', () => {
        it('should generate a deterministic key for a query', () => {
            const key1 = cache_service_1.CacheService.generateKey('company-123', 'test query', 10);
            const key2 = cache_service_1.CacheService.generateKey('company-123', 'test query', 10);
            expect(key1).toBe(key2);
            expect(key1).toContain('company-123');
        });
        it('should generate different keys for different queries', () => {
            const key1 = cache_service_1.CacheService.generateKey('company-123', 'query 1', 10);
            const key2 = cache_service_1.CacheService.generateKey('company-123', 'query 2', 10);
            expect(key1).not.toBe(key2);
        });
        it('should generate different keys for different limits', () => {
            const key1 = cache_service_1.CacheService.generateKey('company-123', 'query', 10);
            const key2 = cache_service_1.CacheService.generateKey('company-123', 'query', 20);
            expect(key1).not.toBe(key2);
        });
        it('should handle filters in key generation', () => {
            const key1 = cache_service_1.CacheService.generateKey('company-123', 'query', 10, { fileId: 'file1' });
            const key2 = cache_service_1.CacheService.generateKey('company-123', 'query', 10, { fileId: 'file2' });
            expect(key1).not.toBe(key2);
        });
    ...
    describe('get', () => {
        it('should return cached data on hit', async () => {
            ...
            const result = await cache_service_1.CacheService.get('test-key');
        ...
        it('should handle invalid JSON gracefully', async () => {
            ...
            await expect(cache_service_1.CacheService.get('test-key')).rejects.toThrow();
        });
    });
    describe('set', () => {
    ...
    describe('invalidateCompany', () => {
        it('should delete all keys for a company', async () => {
            const mockStream = {
                on: jest.fn((event, callback) => {
                    if (event === 'data') {
                        setTimeout(() => callback(['rag_cache:company-123:key1', 'rag_cache:company-123:key2']), 0);
                    }
                    else if (event === 'end') {
            ...
            await cache_service_1.CacheService.invalidateCompany('company-123');
        ...
        it('should handle errors gracefully', async () => {
            ...
            await expect(cache_service_1.CacheService.invalidateCompany('company-123')).resolves.not.toThrow();
        });
    });
    describe('getStats', () => {
        it('should return cache statistics', async () => {
            ...
            expect(cache_service_1.CacheService.getStats).toBeDefined();
