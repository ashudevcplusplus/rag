...
describe('VectorService', () => {
    ...
    beforeEach(() => {
        ...
        js_client_rest_1.QdrantClient.mockImplementation(() => mockQdrant);
    });
    describe('getEmbeddings', () => {
        it('should fetch embeddings successfully', async () => {
            ...
            const result = await vector_service_1.VectorService.getEmbeddings(['text1', 'text2']);
            expect(result).toEqual(mockEmbeddings);
            expect(global.fetch).toHaveBeenCalledWith('http://localhost:5001/embed', expect.objectContaining({
                method: 'POST',
        ...
        it('should handle vectors field name', async () => {
            ...
            const result = await vector_service_1.VectorService.getEmbeddings(['text1']);
        ...
        it('should throw ExternalServiceError on HTTP error', async () => {
            ...
            await expect(vector_service_1.VectorService.getEmbeddings(['text1'])).rejects.toThrow(error_types_1.ExternalServiceError);
        });
        it('should handle network errors', async () => {
            global.fetch.mockRejectedValue(new Error('Network error'));
            await expect(vector_service_1.VectorService.getEmbeddings(['text1'])).rejects.toThrow('Network error');
        });
    });
    describe('ensureCollection', () => {
        it('should do nothing if collection exists', async () => {
            mockQdrant.getCollection.mockResolvedValue({});
            await vector_service_1.VectorService.ensureCollection('test-collection');
        ...
        it('should not throw if payload index creation fails', async () => {
            ...
            await expect(vector_service_1.VectorService.ensureCollection('test-collection')).resolves.not.toThrow();
        });
        it('should throw on non-404 errors', async () => {
            const error = new Error('Server error');
            error.status = 500;
            mockQdrant.getCollection.mockRejectedValue(error);
            await expect(vector_service_1.VectorService.ensureCollection('test-collection')).rejects.toThrow('Server error');
        });
    });
    describe('upsertBatch', () => {
        it('should upsert points successfully', async () => {
            ...
            await vector_service_1.VectorService.upsertBatch('test-collection', points);
        ...
        it('should throw on upsert error', async () => {
            ...
            await expect(vector_service_1.VectorService.upsertBatch('test-collection', points)).rejects.toThrow('Upsert failed');
        });
    });
    describe('search', () => {
        it('should search successfully', async () => {
            ...
            const result = await vector_service_1.VectorService.search('test-collection', [0.1, 0.2, 0.3], 10);
        ...
        it('should include filter in search', async () => {
            ...
            await vector_service_1.VectorService.search('test-collection', [0.1, 0.2], 5, filter);
            expect(mockQdrant.search).toHaveBeenCalledWith('test-collection', {
                vector: [0.1, 0.2],
                limit: 5,
                filter,
        ...
        it('should use default limit of 10', async () => {
            mockQdrant.search.mockResolvedValue([]);
            await vector_service_1.VectorService.search('test-collection', [0.1, 0.2]);
            expect(mockQdrant.search).toHaveBeenCalledWith('test-collection', {
                vector: [0.1, 0.2],
                limit: 10,
                filter: undefined,
        ...
        it('should throw on search error', async () => {
            ...
            await expect(vector_service_1.VectorService.search('test-collection', [0.1, 0.2])).rejects.toThrow('Search failed');
        });
    });
});
