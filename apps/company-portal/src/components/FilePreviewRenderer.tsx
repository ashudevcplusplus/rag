...
export function FilePreviewRenderer({ fileUrl, mimeType, filename }: FilePreviewRendererProps) {
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      try {
        ...
        const response = await fetch(fileUrl, { headers, signal: abortController.signal });
  ...
  // Check error FIRST - otherwise error UI is unreachable when loading=false and blobUrl=null
  if (error) {
    return (
