...
export async function waitForIndexing(jobId: string, timeoutMs = 600000): Promise<IndexingResult> {
  ...
  while (performance.now() < deadline) {
    try {
      const res = await axios.get(`${API_URL}/v1/jobs/${jobId}`, {
      ...
      if (err.response?.status === 429) {
        ...
        console.warn(
          `   ⚠️  Polling error (attempt ${pollCount}): ${err.message} - backing off ${(backoffMs / 1000).toFixed(1)}s`
        );
